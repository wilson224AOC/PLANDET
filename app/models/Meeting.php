...
class Meeting {
    ...
    public function checkConflict($start, $end, $excludeId = null) {
        $query = "SELECT COUNT(*) as count FROM " . $this->table . " WHERE status = 'approved' AND ((scheduled_start < :end AND scheduled_end > :start))";
        if ($excludeId) {
            $query .= " AND id != :id";
        }
        $stmt = $this->conn->prepare($query);
    ...
    public function hasActiveMeetingOnDate(string $dni, string $date): bool {
